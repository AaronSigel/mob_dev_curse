...
- **FR-6**: Обработка `RecoverableSecurityException` при удалении: показ системного диалога и повторная попытка удаления после подтверждения.
- **FR-7**: Сохранение через MediaStore (Scoped Storage), тяжёлые операции (запись, запросы MediaStore) не в UI-потоке.
- **FR-8**: Галерея строится на основе БД Room (uri, тип, дата); при первом запуске выполняется синхронизация из MediaStore; отображаются превью (Coil).
